...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_locations
BEFORE INSERT OR UPDATE ON GLPI_PAU.LOCATIONS
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        SELECT GLPI_PAU.seq_id_locations.nextval INTO :NEW.location_id FROM dual;
    END IF;
    ...
    IF (CHECK_VALUE_EXIST(:NEW."location", '"location"', 'LOCATIONS')) THEN 
        RAISE_APPLICATION_ERROR(-20008, 'Location ' || :NEW."location" || ' already exists in table GLPI_PAU.LOCATIONS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_hardwares
BEFORE INSERT OR UPDATE ON GLPI_PAU.HARDWARES
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        SELECT GLPI_PAU.seq_id_hardwares.nextval INTO :NEW.hardware_id FROM dual;
    END IF;
    --mise en capitales 1ere lettre et les autres en minimuscule
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_users
BEFORE INSERT OR UPDATE ON GLPI_PAU.USERS
FOR EACH ROW
BEGIN
    IF INSERTING THEN
      SELECT GLPI_PAU.seq_id_users.nextval INTO :NEW.user_id FROM dual;
    END IF;
    IF (CHECK_VALUE_EXIST(:NEW.email, 'email', 'USERS')) THEN 
        RAISE_APPLICATION_ERROR(-20009, 'Email ' || :NEW.email || ' already exists in table GLPI_PAU.LOCATIONS');
    END IF;
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_location, 'location_id', 'LOCATIONS') THEN 
        RAISE_APPLICATION_ERROR(-20010, 'Location with ID ' || :NEW.fk_location || ' does not exist in table GLPI_PAU.LOCATIONS');
    END IF;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_tickets
BEFORE INSERT OR UPDATE ON GLPI_PAU.TICKETS
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        SELECT GLPI_PAU.seq_id_tickets.nextval INTO :NEW.ticket_id FROM dual;
    END IF; 
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_hardwares, 'hardware_id', 'HARDWARES') AND :NEW.fk_hardwares <> NULL  THEN
        RAISE_APPLICATION_ERROR(-20018, 'Hardware with ID ' || :NEW.fk_hardwares || ' does not exist in table GLPI_PAU.HARDWARES');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_comments
BEFORE INSERT OR UPDATE ON GLPI_PAU.COMMENTS
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        SELECT GLPI_PAU.seq_id_comments.nextval INTO :NEW.comment_id FROM dual;
    END IF;
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'ticket_id', 'TICKETS') THEN 
        RAISE_APPLICATION_ERROR(-20019, 'Ticket with ID ' || :NEW.fk_ticket || ' does not exist in table GLPI_PAU.TICKETS');
    END IF;
    
    -- V�rifier si la valeur de fk_user existe dans la table USERS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_user, 'user_id', 'USERS') THEN 
        RAISE_APPLICATION_ERROR(-20020, 'User with ID ' || :NEW.fk_user || ' does not exist in table GLPI_PAU.USERS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_ressources
BEFORE INSERT OR UPDATE ON GLPI_PAU.RESSOURCES
FOR EACH ROW
BEGIN
    IF INSERTING THEN
        SELECT GLPI_PAU.seq_id_ressources.nextval INTO :NEW.ressource_id FROM dual;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_ticket_ressources
BEFORE INSERT OR UPDATE ON GLPI_PAU.TICKET_RESSOURCES
FOR EACH ROW
BEGIN
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ressource, 'ressource_id', 'RESSOURCES') THEN 
        RAISE_APPLICATION_ERROR(-20022, 'Ressource with ID ' || :NEW.fk_ressource || ' does not exist in table GLPI_PAU.RESSOURCES');
    END IF;
    
    -- V�rifier si la valeur de fk_ticket existe dans la table TICKETS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'ticket_id', 'TICKETS') THEN 
        RAISE_APPLICATION_ERROR(-20023, 'Ticket with ID ' || :NEW.fk_ticket || ' does not exist in table GLPI_PAU.TICKETS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_comment_ressources
BEFORE INSERT OR UPDATE ON GLPI_PAU.COMMENT_RESSOURCES
FOR EACH ROW
BEGIN
    -- V�rifier si la valeur de fk_ressource existe dans la table RESSOURCES
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ressource, 'ressource_id', 'RESSOURCES') THEN 
        RAISE_APPLICATION_ERROR(-20024, 'Ressource with ID ' || :NEW.fk_ressource || ' does not exist in table GLPI_PAU.RESSOURCES');
    END IF;
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_comment, 'comment_id', 'COMMENTS') THEN 
        RAISE_APPLICATION_ERROR(-20025, 'Comment with ID ' || :NEW.fk_comment || ' does not exist in table GLPI_PAU.COMMENTS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_observers
BEFORE INSERT OR UPDATE ON GLPI_PAU.OBSERVERS
FOR EACH ROW
BEGIN
    -- V�rifier si la valeur de fk_ticket existe dans la table TICKETS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'ticket_id', 'TICKETS') THEN 
        RAISE_APPLICATION_ERROR(-20026, 'Ticket with ID ' || :NEW.fk_ticket || ' does not exist in table GLPI_PAU.TICKETS');
    END IF;
    
    -- V�rifier si la valeur de fk_user existe dans la table USERS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_user, 'user_id', 'USERS') THEN 
        RAISE_APPLICATION_ERROR(-20027, 'User with ID ' || :NEW.fk_user || ' does not exist in table GLPI_PAU.USERS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
...
CREATE OR REPLACE TRIGGER GLPI_PAU.trg_upsert_assigned_to
BEFORE INSERT OR UPDATE ON GLPI_PAU.ASSIGNED_TO
FOR EACH ROW
BEGIN
    -- V�rifier si la valeur de fk_ticket existe dans la table TICKETS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'ticket_id', 'TICKETS') THEN 
        RAISE_APPLICATION_ERROR(-20028, 'Ticket with ID ' || :NEW.fk_ticket || ' does not exist in table GLPI_PAU.TICKETS');
    END IF;
    
    -- V�rifier si la valeur de fk_user existe dans la table USERS
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_user, 'user_id', 'USERS') THEN 
        RAISE_APPLICATION_ERROR(-20029, 'User with ID ' || :NEW.fk_user || ' does not exist in table GLPI_PAU.USERS');
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE;
