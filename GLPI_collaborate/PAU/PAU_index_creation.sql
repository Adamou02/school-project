...
CREATE INDEX GLPI_PAU.idx_tickets_last_modification_date ON GLPI_PAU.TICKETS(last_modification_datetime);
...
CREATE INDEX GLPI_PAU.idx_tickets_resolution_date ON GLPI_PAU.TICKETS(resolution_datetime);
...
CREATE INDEX GLPI_PAU.idx_tickets_closing_date ON GLPI_PAU.TICKETS(closing_datetime);
...
CREATE INDEX GLPI_PAU.idx_tickets_fk_priority ON GLPI_PAU.TICKETS(fk_priority);
...
CREATE INDEX GLPI_PAU.idx_tickets_fk_location ON GLPI_PAU.TICKETS(fk_location);

-- Cet index peut acc�l�rer les requ�tes qui impliquent le tri ou le filtrage par priorit� de ticket, 
-- ce qui est une op�ration courante dans les syst�mes de gestion des tickets.
CREATE INDEX GLPI_PAU.idx_comments_fk_ticket ON GLPI_PAU.COMMENTS(fk_ticket);
...
CREATE BITMAP INDEX GLPI_PAU.idx_users_fk_role ON GLPI_PAU.USERS(fk_role);
...
CREATE BITMAP INDEX GLPI_PAU.idx_users_fk_group ON GLPI_PAU.USERS(fk_group);
...
CREATE BITMAP INDEX GLPI_PAU.idx_tickets_fk_type ON GLPI_PAU.TICKETS(fk_type);
...
CREATE BITMAP INDEX GLPI_PAU.idx_tickets_fk_status ON GLPI_PAU.TICKETS(fk_status);

-- Cet index bitmap peut �tre utile pour les requ�tes qui impliquent la classification des tickets par categorie. 
CREATE BITMAP INDEX GLPI_PAU.idx_tickets_fk_category ON GLPI_PAU.TICKETS(fk_category);

-- Cet index bitmap peut �tre b�n�fique pour les requ�tes qui filtrent ou agr�gent les tickets par groupe assign�.
CREATE BITMAP INDEX GLPI_PAU.idx_tickets_fk_assigned_group ON GLPI_PAU.TICKETS(fk_assigned_group);
...
CREATE INDEX GLPI_PAU.idx_tickets_status_priority ON GLPI_PAU.TICKETS(fk_status, fk_priority);

-- Index composite sur la table COMMENTS pour acc�l�rer les requ�tes filtrant par utilisateur et date de cr�ation.
CREATE INDEX GLPI_PAU.idx_comments_user_creation_date ON GLPI_PAU.COMMENTS(fk_user, creation_datetime);

-- Index composite sur la table USERS pour acc�l�rer les requ�tes filtrant par r�le et entreprise.
CREATE INDEX GLPI_PAU.idx_users_role_company ON GLPI_PAU.USERS(fk_role, company);

-- Index composite sur la table USERS pour optimiser les requ�tes filtrant par groupe et entreprise.
CREATE INDEX GLPI_PAU.idx_users_group_company ON GLPI_PAU.USERS(fk_group, company);

-- Index composite sur la table USERS pour optimiser les requ�tes filtrant par lieu et entreprise.
CREATE INDEX GLPI_PAU.idx_users_location_company ON GLPI_PAU.USERS(fk_location, company);
